...
- **表記ルール** — ONE SLASH は正式表記が要る所（出展者・協力）では「ONE SLASH株式会社」（写真キーも同名）。上田洋平さんの写真（`1-1_上田洋平.png`）は本人確認が取れるまで「まだ待って」（2026-09-20）＝載せない。
- **協力の置き場所** — 木村道徳さんの「協力：琵琶湖博物館 大久保実香・滋賀県立大学 上田洋平」は Day2「ランドスケープ」の企画側（`coop`）に載せる。プロフィールには入れない（CLOSING で登壇者として出るとき「協力」に見えてしまうため。CLOSING は協力ではなく登壇）。
- **題名はマスタが正** — 前川晋也さんの企画は「一輪挿しを日々変えることについて」（「北国街道 木之本宿」はマスタに無いのでサイトにも付けない。ユーザー指示 2026-09-20）。題名・日時・会場・出展者はマスタどおりに書き、こちらで補わない。
