...
## SUMMIT 2025 アーカイブ（耕せ）の記録追加

`/summit/2025` は「土を耕すと記録が芽吹く」体験型アーカイブです。
コンテンツはすべて `src/content/summit2025.ts` の1ファイルで管理しています。

### ワークショップの記録（狙い・振り返り）を書く

1. GitHub で `src/content/summit2025.ts` を開く
2. 対象のワークショップ（`slug` で探す）の `concept`（狙い・コンセプト）と `archive`（当日の振り返り）に、段落ごとの文字列を追加する
...
- 空配列 `[]` のままだと「記録は準備中」の意匠で表示されます
- **注意**: 「言葉の収穫ワークショップ」にはレイアウト確認用のダミー文が入っています。公開前に必ず差し替えてください

### 当日の写真を追加する

1. `public/images/summit2025/ワークショップのslug/` フォルダに画像をアップロード（横4:3推奨、幅1600px程度）
2. `src/content/summit2025.ts` の `photos` に追加する

```ts
photos: [
  { src: "/images/summit2025/kotoba-no-shukaku/01.jpg", caption: "路上に広げた言葉たち" },
],
```
